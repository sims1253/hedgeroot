...
test_that("Priority-based provider order works", {
  providers <- create_test_providers()
  manager <- create_advanced_provider_manager(providers, failover_strategy = "priority")
...
test_that("Round-robin provider order works", {
  providers <- create_test_providers()
  manager <- create_advanced_provider_manager(providers, failover_strategy = "round_robin")
...
test_that("Health-based provider order works", {
  providers <- create_test_providers()
  manager <- create_advanced_provider_manager(providers, failover_strategy = "health_based")
...
test_that("Circuit breaker opens after failures", {
  providers <- create_test_providers()
  manager <- create_advanced_provider_manager(providers, circuit_breaker_threshold = 2)
...
test_that("Manager handles nonexistent provider health updates", {
  ...
  expect_no_error(update_provider_health(manager, "nonexistent", success = TRUE))
